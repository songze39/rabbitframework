...
package com.rabbitframework.commons.org.springframework.io.support;
...
import com.rabbitframework.commons.org.springframework.io.ResourceLoader;
import com.rabbitframework.commons.org.springframework.io.UrlResource;
import com.rabbitframework.commons.org.springframework.io.VfsResource;
import com.rabbitframework.commons.org.springframework.util.AntPathMatcher;
import com.rabbitframework.commons.org.springframework.util.Assert;
import com.rabbitframework.commons.org.springframework.util.PathMatcher;
import com.rabbitframework.commons.org.springframework.util.ReflectionUtils;
import com.rabbitframework.commons.org.springframework.util.SpringResourceUtils;
import com.rabbitframework.commons.org.springframework.util.SpringStringUtils;

/**
 * A {@link ResourcePatternResolver} implementation that is able to resolve a
 * specified resource location path into one or more matching Resources.
 * The source path may be a simple path which has a one-to-one mapping to a
 * target {@link com.rapid.commons.spring.io.Resource}, or alternatively
 * may contain the special "{@code classpath*:}" prefix and/or
 * internal Ant-style regular expressions (matched using Spring's
 * {@link com.rapid.commons.spring.util.AntPathMatcher} utility).
 ...
 * @see com.rapid.commons.spring.util.AntPathMatcher
 * @see com.rapid.commons.spring.io.ResourceLoader#getResource(String)
...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	 ...
	 * @see com.rapid.commons.spring.io.DefaultResourceLoader
	 */
	 ...
	 * @see java.lang.ClassLoader#getResources
	 * @see com.rapid.commons.spring.io.Resource
	 */
	 ...
	 * @see com.rapid.commons.spring.util.PathMatcher
	 */
	protected Resource[] findPathMatchingResources(String locationPattern) throws IOException {
		...
		for (Resource rootDirResource : rootDirResources) {
			...
			else if (rootDirResource.getURL().getProtocol().startsWith(SpringResourceUtils.URL_PROTOCOL_VFS)) {
	 ...
	 * @see #doFindPathMatchingJarResources
	 * @see com.rapid.commons.spring.util.SpringResourceUtils#isJarURL
	 */
	protected boolean isJarResource(Resource resource) throws IOException {
		return SpringResourceUtils.isJarURL(resource.getURL());
	}
	...
	protected Set<Resource> doFindPathMatchingJarResources(Resource rootDirResource, String subPattern)
		...
		if (con instanceof JarURLConnection) {
			...
			SpringResourceUtils.useCachesIfNecessary(jarCon);
			...
			int separatorIndex = urlFile.indexOf(SpringResourceUtils.JAR_URL_SEPARATOR);
			if (separatorIndex != -1) {
				jarFileUrl = urlFile.substring(0, separatorIndex);
				rootEntryPath = urlFile.substring(separatorIndex + SpringResourceUtils.JAR_URL_SEPARATOR.length());
	...
	protected JarFile getJarFile(String jarFileUrl) throws IOException {
		if (jarFileUrl.startsWith(SpringResourceUtils.FILE_URL_PREFIX)) {
			try {
				return new JarFile(SpringResourceUtils.toURI(jarFileUrl).getSchemeSpecificPart());
			}
			catch (URISyntaxException ex) {
				// Fallback for URLs that are not valid URIs (should hardly ever happen).
				return new JarFile(jarFileUrl.substring(SpringResourceUtils.FILE_URL_PREFIX.length()));
			}
		}
		else {
	...
	protected Set<File> retrieveMatchingFiles(File rootDir, String pattern) throws IOException {
		...
		String fullPattern = SpringStringUtils.replace(rootDir.getAbsolutePath(), File.separator, "/");
		if (!pattern.startsWith("/")) {
			fullPattern += "/";
		}
		fullPattern = fullPattern + SpringStringUtils.replace(pattern, File.separator, "/");
	...
	protected void doRetrieveMatchingFiles(String fullPattern, File dir, Set<File> result) throws IOException {
		...
		for (File content : dirContents) {
			String currPath = SpringStringUtils.replace(content.getAbsolutePath(), File.separator, "/");
