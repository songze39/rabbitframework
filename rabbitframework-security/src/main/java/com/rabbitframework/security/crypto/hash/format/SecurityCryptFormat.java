...
/**
 * The {@code Shiro1CryptFormat} is a fully reversible <a
 * href="http://packages.python.org/passlib/modular_crypt_format.html">Modular
 * Crypt Format</a> (MCF). Because it is fully reversible (i.e. Hash -&gt;
 * String, String -&gt; Hash), it does NOT use the traditional MCF encoding
 * alphabet (the traditional MCF encoding, aka H64, is bit-destructive and
 * cannot be reversed). Instead, it uses fully reversible Base64 encoding for
 * the Hash digest and any salt value. <h2>Format</h2>
 * <p>
 * Hash instances formatted with this implementation will result in a String
 * with the following dollar-sign ($) delimited format:
 * </p>
 * 
 * <pre>
 ...
 * <td>The Modular Crypt Format identifier for this implementation, equal to <b>
 * {@code shiro1}</b>. ( This implies that all {@code shiro1} MCF-formatted
 * strings will always begin with the prefix {@code $shiro1$} ).</td>
 * <td>true</td>
 * </tr>
 * <tr>
 * <td>2</td>
 * <td>{@code algorithmName}</td>
 * <td>The name of the hash algorithm used to perform the hash. This is an
 * algorithm name understood by {@code MessageDigest}.
 * {@link java.security.MessageDigest#getInstance(String) getInstance}, for
 * example {@code MD5}, {@code SHA-256}, {@code SHA-256}, etc.</td>
 * <td>true</td>
 * </tr>
 * <tr>
 * <td>3</td>
 ...
 * <td>The Base64-encoded salt byte array. This token only exists if a salt was
 * used to perform the hash.</td>
 * <td>false</td>
 * </tr>
 * <tr>
 * <td>5</td>
 ...
 * @since 1.2
 */
public class SecurityCryptFormat implements ModularCryptFormat,
		ParsableHashFormat {

	public static final String ID = "shiro1";
	public static final String MCF_PREFIX = TOKEN_DELIMITER + ID
			+ TOKEN_DELIMITER;
	...
	public String format(Hash hash) {
		...
		StringBuilder sb = new StringBuilder(MCF_PREFIX).append(algorithmName)
				.append(TOKEN_DELIMITER).append(iterations)
	...
	public Hash parse(String formatted) {
		...
		if (!formatted.startsWith(MCF_PREFIX)) {
			...
			String msg = "The argument is not a valid '" + ID
					+ "' formatted hash.";
