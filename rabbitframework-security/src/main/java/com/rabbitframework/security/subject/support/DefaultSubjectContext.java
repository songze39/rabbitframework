...
/**
 * Default implementation of the {@link SubjectContext} interface. Note that the
 * getters and setters are not simple pass-through methods to an underlying
 * attribute; the getters will employ numerous heuristics to acquire their data
 * attribute as best as possible (for example, if {@link #getPrincipals} is
 * invoked, if the principals aren't in the backing map, it might check to see
 * if there is a subject or session in the map and attempt to acquire the
 * principals from those objects).
 *
 */
public class DefaultSubjectContext extends MapContext implements SubjectContext {
	private static final long serialVersionUID = 1L;
	private static final String className = DefaultSubjectContext.class.getName();
	private static final String SECURITY_MANAGER = className + ".SECURITY_MANAGER";
	...
	public static final String SESSION_CREATION_ENABLED = DefaultSubjectContext.class.getName()
	...
	public boolean resolveAuthenticated() {
		...
		if (authc == null) {
			...
			// presence of one indicates a successful
