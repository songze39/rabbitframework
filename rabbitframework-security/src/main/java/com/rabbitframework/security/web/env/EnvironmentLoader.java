...
import com.rabbitframework.security.config.ResourceConfigurable;
...
/**
 * An {@code EnvironmentLoader} is responsible for loading a web application's
 * Shiro {@link WebEnvironment} (which includes the web app's
 * {@link com.rabbitframework.security.web.mgt.WebSecurityManager
 * WebSecurityManager}) into the {@code ServletContext} at application startup.
 * <p/>
 * In Shiro 1.1 and earlier, the Shiro ServletFilter was responsible for
 * creating the {@code WebSecurityManager} and any additional objects (security
 * filters, etc). However, any component not filtered by the Shiro Filter (such
 * as other context listeners) was not able to easily acquire the these objects
 * to perform security operations.
 * <p/>
 * Due to this, in Shiro 1.2 and later, this {@code EnvironmentLoader} (or more
 * likely, the {@link EnvironmentLoaderListener} subclass) is the preferred
 * mechanism to initialize a Shiro environment. The Shiro Filter, while still
 * required for request filtering, will not perform this initialization at
 * startup if the {@code EnvironmentLoader} (or listener) runs first.
 * <h2>Usage</h2> This implementation will look for two servlet context
 * {@code context-param}s in {@code web.xml}: {@code shiroEnvironmentClass} and
 * {@code shiroConfigLocations} that customize how the {@code WebEnvironment}
 * instance will be initialized.
 * <h3>shiroEnvironmentClass</h3> The {@code shiroEnvironmentClass}
 * {@code context-param}, if it exists, allows you to specify the
 * fully-qualified implementation class name of the {@link WebEnvironment} to
 ...
 * </pre>
 * 
 * If not specified, the default value is the {@link IniWebEnvironment} class,
 * which assumes Shiro's default
 * <a href="http://shiro.apache.org/configuration.html">INI configuration
 * format</a>
 * <h3>shiroConfigLocations</h3> The {@code shiroConfigLocations}
 * {@code context-param}, if it exists, allows you to specify the config
 * location(s) (resource path(s)) that will be relayed to the instantiated
 * {@link WebEnvironment}. For example:
 * 
 * <pre>
 * &lt;context-param&gt;
 ...
 * </pre>
 * 
 * The {@code WebEnvironment} implementation must implement the
 * {@link ResourceConfigurable} interface if it is to acquire the
 * {@code shiroConfigLocations} value.
 * <p/>
 * If this {@code context-param} is not specified, the {@code WebEnvironment}
 * instance determines default resource lookup behavior. For example, the
 * {@link IniWebEnvironment} will check the following two locations for INI
 * config by default (in order):
 * <ol>
 * <li>/WEB-INF/shiro.ini</li>
 * <li>classpath:shiro.ini</li>
 * </ol>
 * <h2>Web Security Enforcement</h2> Using this loader will only initialize
 ...
 * Spring 3's {@code org.springframework.web.context.ContextLoader}
...
public class EnvironmentLoader {
	...
	/**
	 * Servlet Context config param for specifying the {@link WebEnvironment}
	 * implementation class to use: {@code shiroEnvironmentClass}
	 */
	public static final String ENVIRONMENT_CLASS_PARAM = "shiroEnvironmentClass";

	/**
	 * Servlet Context config param for the resource path to use for configuring
	 * the {@link WebEnvironment} instance: {@code shiroConfigLocations}
	 */
	public static final String CONFIG_LOCATIONS_PARAM = "shiroConfigLocations";

	public static final String ENVIRONMENT_ATTRIBUTE_KEY = EnvironmentLoader.class.getName()
			+ ".ENVIRONMENT_ATTRIBUTE_KEY";

	private static final Logger log = LoggerFactory.getLogger(EnvironmentLoader.class);

	/**
	 * Initializes Shiro's {@link WebEnvironment} instance for the specified
	 * {@code ServletContext} based on the {@link #CONFIG_LOCATIONS_PARAM}
	 * value.
	 *
	 ...
	 *             if an existing WebEnvironment has already been initialized
	 *             and associated with the specified {@code ServletContext}.
	 */
	...
	/**
	 * Instantiates a {@link WebEnvironment} based on the specified
	 * ServletContext.
	 * <p/>
	 * This implementation {@link #determineWebEnvironmentClass(ServletContext)
	 * determines} a {@link WebEnvironment} implementation class to use. That
	 * class is instantiated, configured, and returned.
	 * <p/>
	 * This allows custom {@code WebEnvironment} implementations to be specified
	 * via a ServletContext init-param if desired. If not specified, the default
	 * {@link IniWebEnvironment} implementation will be used.
	 *
	 * @param sc
	 *            current servlet context
