...
import com.rabbitframework.security.mgt.DefaultSessionStorageEvaluator;
import com.rabbitframework.security.session.mgt.NativeSessionManager;
...
/**
 * A web-specific {@code SessionStorageEvaluator} that performs the same logic
 * as the parent class {@link DefaultSessionStorageEvaluator} but additionally
 * checks for a request-specific flag that may enable or disable session access.
 * <p/>
 * This implementation usually works in conjunction with the
 * {@link com.rabbitframework.security.web.filter.session.NoSessionCreationFilter}
 * : If the {@code NoSessionCreationFilter} is configured in a filter chain,
 * that filter will set a specific {@code ServletRequest}
 * {@link javax.servlet.ServletRequest#setAttribute attribute} indicating that
 * session creation should be disabled.
 * <p/>
 ...
 * <p/>
 * If the request attribute has not been set (i.e. the
 * {@code NoSessionCreationFilter} is not configured or has been disabled), this
 * class does nothing and delegates to the parent class for existing behavior.
 *
 * @since 1.2
 */
public class DefaultWebSessionStorageEvaluator extends DefaultSessionStorageEvaluator {
	...
	/**
	 * Sets the session manager to use when checking to see if session storage
	 * is possible.
	 * 
	 * @param sessionManager
	 *            the session manager instance for checking.
	 * @since 1.2.1
	 */
	// package protected on purpose to maintain point-version compatibility:
	// (1.2.3 -> 1.2.1 should work always).
