...
public class JDBCRealmTest {
	...
	protected final String salt = username; // Default impl of getSaltForUser
											// returns username
	protected final String testRole = "testRole";
	protected final String testPermissionString = "testDomain:testTarget:testAction";

	// Maps keyed on test method name so setup/teardown can manage per test
	// resources
	...
	public void setup() {
		...
		Ini config = new Ini();
		config.setSectionProperty("main", "myRealm",
				"org.apache.shiro.realm.jdbc.JdbcRealm");
		config.setSectionProperty("main", "myRealmCredentialsMatcher",
				"org.apache.shiro.authc.credential.Sha256CredentialsMatcher");
		config.setSectionProperty("main", "myRealm.credentialsMatcher",
				"$myRealmCredentialsMatcher");
		config.setSectionProperty(
				"main",
				"securityManager.sessionManager.sessionValidationSchedulerEnabled",
				"false");
	...
	public void testPermissionNotPresent() throws Exception {
		...
		UsernamePasswordToken token = new UsernamePasswordToken(username,
				plainTextPassword);
		currentUser.login(token);
		Assert.assertFalse(currentUser
				.isPermitted("testDomain:testTarget:specialAction"));
	}
	...
	protected void createRealm(String testMethodName) {
		JdbcRealm realm = (JdbcRealm) securityManager.getRealms().iterator()
				.next();
	...
	/**
	 * Creates a test database with the default (no separate salt column)
	 * schema, salting with username if salted is true. Sets the DataSource of
	 * the realm associated with the test to a DataSource connected to the
	 * database. (To prevent concurrency problems when tests are executed in
	 * multithreaded mode, each test method gets its own database.)
	 */
	protected void createDefaultSchema(String testName, boolean salted) {
		...
		try {
			...
			Sha256Hash sha256Hash = salted ? new Sha256Hash(plainTextPassword,
					salt) : new Sha256Hash(plainTextPassword);
	 ...
	 * Sets the DataSource of the realm associated with the test to a DataSource
	...
	protected void createRolesAndPermissions(DataSource ds) {
		...
		try {
			...
			sql.executeUpdate("insert into user_roles values ('" + username
					+ "', '" + testRole + "')");
			sql.executeUpdate("create table roles_permissions (role_name varchar(20), permission varchar(40))");
			sql.executeUpdate("insert into roles_permissions values ('"
					+ testRole + "', '" + testPermissionString + "')");
		} catch (SQLException ex) {
